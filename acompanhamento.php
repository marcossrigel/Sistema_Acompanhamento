...
/* 1) Pega a linha de referência (de onde o usuário clicou “Acompanhamento”) */
...
/* 2) Usa (id_usuario, sei, codigo) como chave do grupo da mesma demanda */
$idUsuario = (int)$ref['id_usuario'];
$sei       = $ref['sei'];
$codigo    = $ref['codigo'];
...
$st = $conn->prepare($sql);
$st->bind_param("iss", $idUsuario, $sei, $codigo);
$st->execute();
$hist = $st->get_result()->fetch_all(MYSQLI_ASSOC);
$st->close();

/* 3) Mapear última ocorrência de cada setor e descobrir qual está em andamento */
$ultimoPorSetor = [];        // setor => linha (a última)
$setorAtual = null;          // setor com data_liberacao NULL (se houver)
$primeiraLinha = $hist[0] ?? null; // “Demandante” (criação)

foreach ($hist as $row) {
  $set = trim($row['setor_responsavel']);
  $ultimoPorSetor[$set] = $row;
  if ($row['data_liberacao'] === null) {
    $setorAtual = $set;
  }
}

/* 4) Defina a ordem do fluxo (use os mesmos nomes que vêm no banco)  */
$steps = [
  ['key' => 'DEMANDANTE',  'label' => 'Demandante',            'hint' => 'Recebido'],
  ['key' => 'DAF - DIRETORIA DE ADMINISTRAÇÃO E FINANÇAS', 'label' => 'DAF',        'hint' => 'Recebido'],
  ['key' => 'GECOMP',      'label' => 'GECOMP',                'hint' => 'Análise'],
  ['key' => 'DDO',         'label' => 'DDO',                   'hint' => 'Execução'],
  ['key' => 'CPL',         'label' => 'CPL',                   'hint' => 'Aguardando'],
  ['key' => 'HOMOLOGACAO', 'label' => 'Homologação',           'hint' => 'Aguardando'],
  ['key' => 'PARECER JUR', 'label' => 'Parecer Jur.',          'hint' => 'Aguardando'],
  ['key' => 'NE',          'label' => 'NE',                    'hint' => 'Aguardando'],
  ['key' => 'PF',          'label' => 'PF',                    'hint' => 'Aguardando'],
  ['key' => 'NE',          'label' => 'NE',                    'hint' => 'Aguardando'], // segunda NE do seu fluxo
  ['key' => 'LIQ',         'label' => 'LIQ',                   'hint' => 'Aguardando'],
  ['key' => 'PD',          'label' => 'PD',                    'hint' => 'Aguardando'],
  ['key' => 'OB',          'label' => 'OB',                    'hint' => 'Aguardando'],
  ['key' => 'REMESSA',     'label' => 'Remessa',               'hint' => 'Aguardando'],
];

/* 5) Descobrir o índice “current” (em andamento) */
$currentIdx = -1;
if ($setorAtual !== null) {
...
/* 6) Para cada step definimos status (done/current/todo) e datas */
$cards = [];
foreach ($steps as $i => $s) {
  $key   = $s['key'];
  $label = $s['label'];
  $hint  = $s['hint'];
  $stRow = $ultimoPorSetor[$key] ?? null;

  // “Demandante”: usa a primeira linha do histórico
  if ($key === 'DEMANDANTE') {
    $stRow = $primeiraLinha;
  }
  ...
  $status = 'todo';
  if ($key === 'DEMANDANTE') {
    $status = 'done'; // sempre concluído (criação feita)
  } else if ($currentIdx >= 0) {
    if ($i < $currentIdx)       $status = 'done';
    else if ($i === $currentIdx)$status = 'current';
    else                        $status = 'todo';
  } else {
    // não há ninguém “current”: então tudo que existe no histórico está done
    if ($stRow && $stRow['data_liberacao'] !== null) $status = 'done';
    else $status = 'todo';
  }

  // frase pequena abaixo do título
  if ($key === 'DEMANDANTE') {
...
/* 7) Progresso do trilho: até o índice “current” (ou último done) */
$progressIdx = -1;
...
// porcentagem aproximada no grid (sem medir DOM)
...
<style>
  :root{
    --bg:#eef3f7; --ink:#223; --muted:#6b7280;
    --done:#16a34a; --doing:#f59e0b; --todo:#9ca3af;
    --card:#fff; --rail:#e5e7eb; --rail-fill:#2563eb;
    --shadow:0 8px 24px rgba(0,0,0,.06); --radius:18px;
  }
  *{box-sizing:border-box}
  body{margin:0; font-family:Poppins,system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Arial; background:var(--bg); color:var(--ink);}
  ...
  .timeline{background:var(--card); border-radius:24px; box-shadow:var(--shadow); padding:22px; overflow:hidden;}
  ...
  .progress{height:100%; width:<?= (int)$progressPct ?>%; background:linear-gradient(90deg, var(--rail-fill), #3b82f6); border-radius:999px; transition:width .35s ease;}
  .step{border:1px solid #eef; border-radius:var(--radius); padding:16px 16px 18px; background:#fff; box-shadow:0 1px 0 rgba(0,0,0,.03);}
  ...
  .dot{width:20px; height:20px; border-radius:999px; display:grid; place-items:center; font-size:12px; border:2px solid currentColor;}
